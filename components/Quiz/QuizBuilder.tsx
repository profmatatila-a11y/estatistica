        ...
        <div className="p-6 max-w-5xl mx-auto space-y-8">
            <div className="flex justify-between items-center">
                ...
                {generatedLink && (
                    <div className="flex items-center gap-2 bg-green-50 px-4 py-2 rounded-lg border border-green-200">
                        <span className="text-green-700 text-sm font-medium truncate max-w-xs">{generatedLink}</span>
                        <button onClick={copyLink} className="text-green-700 font-bold hover:underline ml-2">Copiar</button>
                    </div>
                )}
            </div>

            <div className="space-y-6 bg-white p-8 rounded-xl shadow-sm border border-slate-200">
                <h2 className="text-lg font-bold text-slate-700 border-b pb-2 mb-4">Configurações Gerais</h2>

                {/* Branding Preview */}
                <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 mb-6">
                    <p className="text-center font-bold text-slate-400 text-xs uppercase mb-2">Simulação do Cabeçalho</p>
                    <div className="text-center">
                        <h1 className="text-xl font-bold text-slate-900">Matemática Online</h1>
                        <h2 className="text-md text-slate-700 font-medium">Prof. Átila de Oliveira</h2>
                        <div className="mt-2 text-sm text-slate-600 italic min-h-[1.5rem] whitespace-pre-wrap">{customHeader || 'Seus dizeres aparecerão aqui...'}</div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                        ...
                        <textarea
                            className="w-full border-slate-300 rounded-md shadow-sm p-3 border"
                            value={customHeader}
                    ...
                    <div key={q._tempId || q.id || qIdx} className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 relative group">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex gap-4 items-center">
                                <select
                                    className="border-slate-300 rounded-md shadow-sm p-2 text-sm border font-medium text-slate-700 bg-slate-50"
                                    value={q.type || 'multiple_choice'}
                                    onChange={e => updateQuestion(qIdx, 'type', e.target.value)}
                                >
                                    <option value="multiple_choice">Múltipla Escolha</option>
                                    <option value="text">Discursiva (Texto)</option>
                                </select>
                                <button
            ...
            <div className="flex gap-4 sticky bottom-6 bg-white/80 backdrop-blur-sm p-4 rounded-xl border border-slate-200 shadow-lg">
                ...
                >
                    ...
                    {loading ? 'Gerando Link...' : 'Salvar e Gerar Link'}
                </button>
            </div>
        </div>
    );
